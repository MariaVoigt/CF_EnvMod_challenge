---
title: "Climate Farmers coding challenge"
author: "Maria Voigt"
date: '`r format(Sys.time(), "%d\\. %B %Y")`'
output:
 html_document:
   includes:
   css: "./aux/report_tab.css"
   highlight: textmate  # Syntax highlighting style
   number_sections: yes
   toc: TRUE
   toc_float:
     collapsed: FALSE
---
<div class="vspace_big"></div>
<img src="./aux/cropped_climate_farmers.png" alt="Red Line" class="img_center" width= 10% height=5%></img>
<div class="vspace_small"></div>
<p style="font-size:2em; font-weight:100; text-align: center">Coding challenge for Climate Farmers</p>
<div class="vspace_small"></div>


```{r global-options, include=FALSE}
# load packages ---------------------------------------------------------

# basic
library(tidyverse)   # Collection of R packages designed for data science
library(rlang)       # A toolbox for working with base type (e.g. environments)
library(htmltools)   # Tools for HTML generation and output

library(sf)           # Comprehensive toolset for handling spatial data
library(terra)        # Efficient manipulation & analysis of raster data
library(tidyterra)    # Extends 'terra' for tidyverse-friendly raster operations
library(ncdf4)        # Reading/writing data in netCDF format
options("sp_evolution_status" = 2) 
library(exactextractr) # extraction of raster values based on geometries and resampling
library(here)
here::i_am("src/markdown/Coding_challenge.Rmd")

# set knitr options ------------------------------------------------------------
options(scipen = 999)

# Scheme for ggplot  ---------------------------------------------------------

theme_simple <- function() {
theme_bw()+
theme(
  panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(),
  panel.background = element_blank(), 
  axis.text.y = element_text(size = 12),
  axis.text.x=element_text(size = 12, angle = 90, hjust=0.5,vjust=0.2),
  axis.title=element_text(size = 18, face = "bold"),
  strip.background=element_rect(colour="#f0f0f0",fill="#f0f0f0"),
  strip.text = element_text(size = 16, face="bold"))
}

```


# Region Selection and Data Aqcuisition


## Region selection and basemap preparation 


I decided to work with Portugal, because it includes various types of landcover,
and has variability in soil organic carbon and climate. I imported the country shape from the gadm database [GADM database](https://gadm.org/download_country.html).
As I want to work with mainland Portugal, I excluded Madeira and the Azores.

```{r load_data_prep, include = FALSE}
# Load results for data prep ----------------------------------------------------
local_env_1<- env()
source(here("src/analysis/1_read_and_prep_data.R"), local = local_env_1, encoding = "UTF-8")

```

```{r basemap, fig.asp = 0.8, fig.width = 10, echo = FALSE, fig.cap="Shape of mainland Portugal used to restrict the analysis of soil, landcover and climate data."}
ggplot() + 
  geom_sf(data=sf::st_as_sf(local_env_1$pt_mainland), color = "black", 
          fill = "grey")+
  theme_simple()

```
</div>
<div class="vspace_small"></div>

## Data Aqcuisition {.tabset}

I downloaded the following datasets for the analysis:

<ol>
  <li>
    <strong>Climate (years 2020-2022):</strong>
    <ul>
      <li>
        <em>Temperature (unit: Kelvin):</em><br>
        Air temperature 2 meters above land or water surfaces.
      </li>
      <li>
        <em>Evapotranspiration (unit: Meters):</em><br>
        Accumulated amount of water that has evaporated from the Earth's surface, including a simplified representation of transpiration from vegetation.
      </li>
      <li>
        <em>Precipitation (unit: Meters):</em><br>
        Accumulated liquid and frozen water (rain, snow) falling to Earth's surface, excluding fog, dew, and evaporated precipitation. It is the depth the water would have if it were spread evenly over the grid box.
      </li>
    </ul>
  </li>
  <li>
    <strong>Landcover (year 2020):</strong>
    <ul>
      <li>
        <em>Global gridded land cover classification:</em><br>
        Land cover class per pixel, according to a legend of 22 classes, defined using the Land Cover Classification System developed by the United Nations Food and Agriculture Organization. 
      </li>
    </ul>
  </li>
  <li>
    <strong>Soil organic carbon (SOC):</strong>
    <ul>
      <li>
        <em>Soil organic carbon (unit: t/ha):</em><br>
        Soil organic carbon content in 0-30 cm depth.
      </li>
    </ul>
  </li>
</ol>


Each layer was clipped with the shape of mainland Portugal for the analysis. 

### Temperature
```{r temperature_original, fig.asp = 0.8, fig.width = 10, echo = FALSE, return = FALSE, fig.cap="Temperature in Portugal for January 2020."}
ggplot() +
  geom_spatraster(data = local_env_1$temperature_pt$t2m_1)   +
  scale_fill_gradient(name = "Temperature (°C)", 
                      low = "blue", high = "red", na.value = "white") +  # 
   theme_simple()
```

### Evapotranspiration
```{r evapotransp_original, fig.asp = 0.8, fig.width = 10, echo = FALSE, return = FALSE, fig.cap="Evapotranspiration in Portugal for January 2020."}
ggplot() +
  geom_spatraster(data = local_env_1$evapotransp_pt$e_1)   +
  scale_fill_gradient(name = "Evapotranspiration (m)", 
                      low = "lightgrey", high = "gray10", na.value = "white") +
   theme_simple()
```

### Precipitation
```{r precip_original, fig.asp = 0.8, fig.width = 10, echo = FALSE, return = FALSE, fig.cap="Precipitation in Portugal for January 2020."}
ggplot() +
  geom_spatraster(data = local_env_1$precipitation_pt$tp_1)   +
  scale_fill_gradient(name = "Precipitation (m)", 
                      low = "lightblue", high = "darkblue", na.value = "white") +
  theme_simple()
```

### Landcover
```{r landcover_original, fig.asp = 0.8, fig.width = 10, echo = FALSE, return = FALSE, message=FALSE, warning=FALSE, fig.cap="<span class='caption'>Land cover classes in Portugal in 2020.</span>"}

# source functions to visualise landcover
source(here("src/functions/0_visualise_landcover_classes.R"))
landcover_pt <- landcover_categories(local_env_1$landcover_pt, 3)

landcover_classes_exist <- existing_landcover_classes(local_env_1$landcover_pt, 3)

ggplot() +
  geom_spatraster(data = landcover_pt, mapping = aes(fill = label) )   +
  scale_fill_manual(
    values = landcover_classes_exist$hex,
    na.value = "white")+
   labs(fill = NULL) +
  theme_simple() + 
   theme(legend.position = "bottom",
         legend.text = element_text(size = 6))
```

The  label and color information was extracted from the metadata of the land cover layer. However, I decided to change the color for the three cropland rainfed classes 
slightly, so they could be distinguished better.

### Soil
```{r soil_original, fig.asp = 0.8, fig.width = 10, echo = FALSE, return = FALSE, fig.cap="Soil organic carbon content in Portugal."}
# make custom palette
ggplot() +
  geom_spatraster(data = local_env_1$SOC_pt)   +
 scale_fill_gradientn(name = "Soil organic carbon (t/ha)",
                      colors = c("#4c0073", "#2a895a", "#ffffbe",
                                  na.value = "white"))+
   theme_simple()
```
</div>
<div class="vspace_small"></div>


# Data integration
# SOMETHING broke here fix it
I extracted the resolution and extent of all layers. 

<table style="border-collapse: collapse;">
  <tr>
    <th style="padding-right: 40px; border: 1px solid black;">Layer Name</th>
    <th style="padding-right: 40px; border: 1px solid black;">Resolution (lon)</th>
    <th style="border: 1px solid black;">Resolution (lat)</th>
    <th style="padding-right: 40px; border: 1px solid black;">extent (lon)</th>
    <th style="border: 1px solid black;">extent (lat)</th>
  </tr>
  <tr>
    <td style="border: 1px solid black;">Climate variables</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$evapotransp_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$evapotransp_pt)[2], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$evapotransp_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$evapotransp_pt)[2], 4)`</td>
        
  </tr>
  <tr>
    <td style="border: 1px solid black;">Land Cover</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$landcover_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$landcover_pt)[2], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$landcover_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$landcover_pt)[2], 4)`</td>
  </tr>
  <tr>
    <td style="border: 1px solid black;">Soil Organic Carbon</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$SOC_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(res(local_env_1$SOC_pt)[2], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$SOC_pt)[1], 4)`</td>
    <td style="border: 1px solid black;">`r round(ext(local_env_1$SOC_pt)[2], 4)`</td>
  </tr>
</table>

<div class="vspace_small"></div>

The climate variables (which share the same spatial dimensions) doesn't match with the land cover and SOC layer. Therefore we have to resample the latter layers for analysis to the same resolution and extent as the climate variables. 



## Data resampling {.tabset}
## Landcover data integration
Showing the different options
also have to think here how I load the things into that script
whether I save the portugal layers somewhere or just load them from
the source file

## Soil organic carbon data integration
Showing the different options

# Analysis and Visualization

# Sampling scheme


# Soil model



